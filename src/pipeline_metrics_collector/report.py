...
from pipeline_metrics_collector.collector import MetricsCollection, collection_status
from pipeline_metrics_collector.contract import (
    MetricsReport,
    metrics_report_from_dict,
    metrics_report_to_dict,
)
from pipeline_metrics_collector.version import __version__
...
def read_metrics_report(
    ...
    if not text.strip():
        raise ValueError(f"Metrics report is empty: {report_path}")

    try:
        payload = json.loads(text)
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid metrics report JSON: {exc.msg}") from exc

    if not isinstance(payload, dict):
        raise ValueError("Metrics report must contain a JSON object")

    validation_errors = validate_metrics_report_data(payload)

    if validation_errors:
        raise ValueError("Invalid metrics report: " + "; ".join(validation_errors))
...
def validate_metrics_report_data(
    ...
    if "collector_version" in payload:
        collector_version = payload["collector_version"]

        if not isinstance(collector_version, str):
            errors.append("collector_version must be a string")
        elif not collector_version.strip():
            errors.append("collector_version must not be empty")
    ...
    if "status" in payload and payload["status"] not in {
        ...
        errors.append("status must be completed, partial or empty")
    ...
    if metrics is not None and not isinstance(
        ...
        for index, metric in enumerate(metrics):
            ...
            if "name" not in metric:
                errors.append(f"metrics[{index}] is missing name")

            if "value" not in metric:
                errors.append(f"metrics[{index}] is missing value")
...
def build_metrics_report_inspection(
    ...
    return {
        ...
        "summary": {
            ...
            "execution_requested": (report.summary.execution_requested),
            "execution_completed": (report.summary.execution_completed),
            "runtime_failed": (report.summary.runtime_failed),
            "accepted_events": (report.summary.accepted_events),
            "rejected_events": (report.summary.rejected_events),
